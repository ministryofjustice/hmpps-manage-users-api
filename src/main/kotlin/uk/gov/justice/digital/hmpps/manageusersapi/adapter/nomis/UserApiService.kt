...
class UserApiService(
  ...
  @Qualifier("nomisUserWebExtendedTimeoutClientUtils") val userWebClientExtendedTimeoutUtils: WebClientUtils,
) {
  companion object {
  ...
  fun downloadUsersByFilter(filter: PrisonUserFilter) = userWebClientExtendedTimeoutUtils.getWithParams(
    "/users/download",
    object : ParameterizedTypeReference<List<PrisonUserSummary>>() {},
    mapPrisonUserFilterToMap(filter),
  )

  fun downloadPrisonAdminsByFilter(filter: PrisonUserFilter) = userWebClientExtendedTimeoutUtils.getWithParams(
